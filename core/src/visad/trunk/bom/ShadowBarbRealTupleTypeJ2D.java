...
/**
   The ShadowBarbRealTupleTypeJ2D class shadows the RealTupleType class for
   BarbManipulationRendererJ2D, within a DataDisplayLink, under Java2D.<P>
*/
public class ShadowBarbRealTupleTypeJ2D extends ShadowRealTupleTypeJ2D {
  ...
  public boolean doTransform(VisADGroup group, Data data, float[] value_array,
    ...
    int LevelOfDifficulty = getAdaptedShadowType().getLevelOfDifficulty();
    if (LevelOfDifficulty == NOTHING_MAPPED) return false;

    if (!(data instanceof RealTuple)) {
      throw new DisplayException("data must be RealTuple: " +
                                 "ShadowBarbRealTupleTypeJ2D.doTransform");
    }
 
    // copy from ShadowTupleTypeJ2D
    // . . .
