...
public class GriddedSet extends SampledSet {
  ...
  Index
  {
    ...
    private final int[]	lengths = getLengths();
    private final int[]	roll = new int[lengths.length];
    private int		index = 0;
    private int[]	indexes;
    protected Index()
      throws VisADException
    {
      maxIndex = getLength() - 1;
      if (lengths.length > 0)
	roll[0] = 1;
    ...
    {
      return indexes[idim] <= 0 ? -1 : index - roll[idim];
    }
	...
	return false;
      index++;
      for (int idim = 0;
	idim < lengths.length && ++indexes[idim] >= lengths[idim]; )
      {
	indexes[idim++] = 0;
      }
      return true;
   ...
   * same as this Set's.  All components of the curve integral at the first
   * point shall be arbitrarily set to zero.
   *
   ...
   *
   * Derived classes may override this method for faster performance.
   * This method, itself, takes approximately 830/845 the time of
   * SampledSet.curveIntegralOfGradient(float[][][], float[][]) -- which
    ...
    float[][] newRangeValues) throws VisADException
  {
    ...
    /*
     * Arbitrarily set all components of the first sample point to zero.
     */
    for (int icomp = 0; icomp < componentCount; ++icomp)
      newRangeValues[icomp][0] = 0;
    /*
    ...
    while (index.increment())
    {
      ...
      float[][]	sampleCoordinates = indexToValue(new int[] {sampleIndex});
	...
	if (neighborIndex >= 0)
	{
	  float[][]	neighborCoordinates =
	    indexToValue(new int[] {neighborIndex});
	  float	deltaDomain =
	    sampleCoordinates[idim][0] - neighborCoordinates[idim][0];
	  /*
	  ...
	  {
	    /*
             * Compute the contribution due to this neighboring point.  Weight
             * the contribution by the reciprocal of the estimated variance.
             * Estimate the variance from the difference between the linear and
             * quadratic estimates.
	     */
	    ...
	    double	delta2 = deltaDomain *
	      (derivatives[sampleIndex] + neighborDerivative) / 2;
	    double	error = delta1 - delta2;
	    double	weight = 1 / (error * error);
	    if (Double.isInfinite(weight))
	    {
	      infinitySums[icomp] += newRangeValues[icomp][neighborIndex] +
		delta2;
	      infinityCounts[icomp]++;
	    }
	    else if (infinityCounts[icomp] == 0)
	    {
	      valueSums[icomp] += (newRangeValues[icomp][neighborIndex] +
		  delta2) * weight;
	...
	newRangeValues[icomp][sampleIndex] = 
	  infinityCounts[icomp] != 0
	    ? (float)(infinitySums[icomp] / infinityCounts[icomp])
	    : weightSums[icomp] != 0
