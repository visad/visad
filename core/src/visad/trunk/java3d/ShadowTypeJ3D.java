...
public abstract class ShadowTypeJ3D extends ShadowType {
  ...
  private static java.util.Hashtable appearanceCache =
    new java.util.Hashtable();

  /** This holds the cached java3d ColoringAttributes */
  private static java.util.Hashtable coloringAttributesCache =
    new java.util.Hashtable();

  /** This holds the cached java3d TransparencyAttributes */
  private static java.util.Hashtable transparencyAttributesCache =
    new java.util.Hashtable();
   ...
   *
   * @param type  MathType of the data
   * @param link  the data/display link
   * @param parent  parent ShadowType
   *
   * @throws RemoteException  problem creating remote instance
   * @throws VisADException   problem creating local instance
   */
   ...
   *
   * @param parent  the parent object to test
   *
  ...
  public int checkIndices(int[] indices, int[] display_indices,
                          int[] value_indices, boolean[] isTransform,
                          int levelOfDifficulty)
          throws VisADException, RemoteException {
    return adaptedShadowType.checkIndices(
             indices, display_indices, value_indices, isTransform,
             levelOfDifficulty);
  }
   ...
   * @param constant_color  the  constant color value
   * @param geometry  the GeometryArray
   * @param no_material  flag for material
   *
   * @return
   */
  public Appearance makeAppearance(GraphicsModeControl mode,
                                   ...
                                   boolean no_material) {
    return ShadowTypeJ3D.staticMakeCachedAppearance(
             mode, constant_alpha, constant_color, geometry, no_material,
             false);
  }

  /**
   * Make an Appearance that may be cached or not, depending on the 
   * okToCache and the cache flag
   *
   * @param mode   GraphicsModeControl
   * @param constant_alpha  the constant alpha value
   * @param constant_color  the  constant color value
   * @param geometry  the GeometryArray
   * @param no_material  flag for material
   * @param okToCache  flag for caching checked with mode.getCacheAppearances
   *
   * @return
   */
  ...
  /**
   * A utility method to add the given geometry into the given group with 
   * the given appearance. If the mergeShapes flag is true 
   * then this will look for an existing child Shape3D node of the group 
   * that has the same Appearance and contains geometries of the same type. 
   * If found it will add the geometry to that Shape3D node.
   * If not found or if mergeShapes is false then this behaves normally, 
   * creating a new Shape3D node and adding  it to the group.
   *
   * @param group     group to add to
  ...
  /**
   * A utility method to create a ColoringAttribute with the given colors
   * If the cacheAppearances flag is true then this will try to find
   * the ColoringAttribute in the cache. If false it just creates a new one.
  ...
  /**
   * A utility method to create a TransparencyAttributes with the given 
   * mode and value. If the flag cacheAppearances is true then this will 
   * try to find the TransparencyAttributes in the cache. If false it 
   * just creates a new one.
   * Note: If caching is on then the result object should be treated as being
   * immutable
   *
   * @param mode  Transparency mode
   * @param value transparancy value
   *
   * @return
   */
  private TransparencyAttributes getTransparencyAttributes(int mode,
          float value) {
    String key = null;
  ...
  public static Appearance staticMakeAppearance(GraphicsModeControl mode,
    ...
    return staticMakeCachedAppearance(
             mode, constant_alpha, constant_color, geometry, no_material,
             false);
  }
   ...
   * @param  no_material  true to not use a Material for illumination,
   *                      false to use it for 2-D geometries
   * @param okToCache If true and if the mode's cacheAppearances flag is
   *                  true then we will use the appearance cache.
   *
  ...
  private static Appearance staticMakeCachedAppearance(GraphicsModeControl mode,
    ...
    try {
      java.lang.reflect.Method method =
        polygon.getClass().getMethod("setPolygonOffsetFactor",
    ...
    rendering.setCapability(
      RenderingAttributes.ALLOW_ALPHA_TEST_FUNCTION_READ);
...
*/
  ...
  public VisADGeometryArray[] assembleShape(float[][] display_values,
                                            ...
                                            int index, ShadowType shadow_api)
          throws VisADException, RemoteException {
    return adaptedShadowType.assembleShape(
             display_values, valueArrayLength, valueToMap, MapVector,
             valueToScalar, display, default_values, inherited_values,
             spatial_values, color_values, range_select, index, shadow_api);
  }
  ...
  public Set assembleSpatial(float[][] spatial_values,
                             ...
                             Set domain_set, boolean allSpatial,
                             boolean set_for_shape, int[] spatialDimensions,
                             boolean[][] range_select,
                             float[][] flow1_values, float[][] flow2_values,
                             float[] flowScale, boolean[] swap,
                             DataRenderer renderer, ShadowType shadow_api)
          throws VisADException, RemoteException {
    return adaptedShadowType.assembleSpatial(
             spatial_values, display_values, valueArrayLength, valueToScalar,
             display, default_values, inherited_values, domain_set,
             allSpatial, set_for_shape, spatialDimensions, range_select,
  ...
  public void assembleFlow(float[][] flow1_values, float[][] flow2_values,
                           float[] flowScale, float[][] display_values,
                           int valueArrayLength, int[] valueToScalar,
                           DisplayImpl display, float[] default_values,
                           boolean[][] range_select, DataRenderer renderer,
                           ShadowType shadow_api)
          throws VisADException, RemoteException {
    adaptedShadowType.assembleFlow(
      flow1_values, flow2_values, flowScale, display_values,
  ...
  public VisADGeometryArray[] makeFlow(int which, float[][] flow_values,
                                       ...
                                       boolean[][] range_select)
          throws VisADException {
    return adaptedShadowType.makeFlow(
             which, flow_values, flowScale, spatial_values, color_values,
             range_select);
  }
  ...
  public VisADGeometryArray[] makeStreamline(int which,
                                             ...
                                             Vector MapVector)
          throws VisADException {
    return adaptedShadowType.makeStreamline(
             which, flow_values, flowScale, spatial_values, spatial_set,
             spatialManifoldDimension, color_values, range_select,
  ...
  public boolean makeContour(int valueArrayLength, int[] valueToScalar,
                             ...
                             int[] inherited_values, Vector MapVector,
                             int[] valueToMap, int domain_length,
                             boolean[][] range_select,
                             int spatialManifoldDimension, Set spatial_set,
                             byte[][] color_values, boolean indexed,
                             Object group, GraphicsModeControl mode,
                             boolean[] swap, float constant_alpha,
                             float[] constant_color, ShadowType shadow_api,
          ...
          throws VisADException {
    return adaptedShadowType.makeContour(
             valueArrayLength, valueToScalar, display_values,
             inherited_values, MapVector, valueToMap, domain_length,
             range_select, spatialManifoldDimension, spatial_set,
             color_values, indexed, group, mode, swap, constant_alpha,
             constant_color, shadow_api, Domain, DomainReferenceComponents,
  ...
  public void addLabelsToGroup(Object group, VisADGeometryArray[][] arrays,
    ...
    for (int ii = 0; ii < n_labels; ii++) {
      ...
      if (control.getAutoSizeLabels()) {
        LabelTransform lbl_trans = new LabelTransform(lbl_trans_group,
                                     p_cntrl, lbl_arrays, f_array[0][ii], 0);
      ...
      addToGroup(
        lbl_trans_group, lbl_arrays[0], mode, constant_alpha, constant_color);
  ...
  public VisADGeometryArray makeText(String[] text_values,
                                     ...
                                     boolean[][] range_select)
          throws VisADException {
    return adaptedShadowType.makeText(
             text_values, text_control, spatial_values, color_values,
             range_select);
  }
  ...
  public byte[][] assembleColor(float[][] display_values,
                                ...
                                ShadowType shadow_api)
          throws VisADException, RemoteException {
    return adaptedShadowType.assembleColor(
  ...
  public boolean[][] assembleSelect(float[][] display_values,
                                    ...
                                    ShadowType shadow_api)
          throws VisADException {
    return adaptedShadowType.assembleSelect(
             display_values, domain_length, valueArrayLength, valueToScalar,
             display, shadow_api);
  }
  ...
  public boolean terminalTupleOrScalar(Object group,
    ...
    boolean post = adaptedShadowType.terminalTupleOrScalar(group,
                     display_values, text_value, text_control,
                     valueArrayLength, valueToScalar, default_values,
                     inherited_values, renderer, this);
  ...
  public static void ensureNotEmpty(Object obj, DisplayImpl display) {
    ...
    Appearance appearance =
      staticMakeCachedAppearance(display.getGraphicsModeControl(), null,
                                 null, geometry, false, true);
   ...
   * @param mode   GraphicsModeControl
   * @param constant_alpha  constant alpha value
   * @param constant_color  constant color value
   *
   * @return  true if successful
   *
   * @throws VisADException   unable to add the array to the group
   */
  public boolean addToGroup(Object group, VisADGeometryArray array,
    ...
    if (array != null && array.vertexCount > 0) {
      ...
      if (constant_alpha == 1.0f) {
        ...
        c_alpha = getTransparencyAttributes(TransparencyAttributes.NONE,
                                            0.0f);
      }
      ...
      if (constant_color != null && constant_color.length == 3) {
        ...
        if (!(array instanceof VisADLineArray ||
              ...
              array instanceof VisADLineStripArray) && array.colors == null) {
          int color_len = 3;
          if (af != 0.0f) {
            color_len = 4;
          }
  ...
  public boolean addTextToGroup(Object group, VisADGeometryArray array,
    ...
    if (array != null && array.vertexCount > 0) {
      ...
      if (constant_alpha == 1.0f) {
        ...
        // WLH - 18 Aug 99 - how could this have gone undetected for so long?
        // c_alpha = getTransparencyAttributes(mode.getTransparencyMode(), 1.0f);
        c_alpha = getTransparencyAttributes(mode.getTransparencyMode(), 0.0f);
      }
      ColoringAttributes c_color = null;
      if (constant_color != null && constant_color.length == 3) {
...
class LabelTransform {
  ...
  LabelTransform(TransformGroup trans, ProjectionControl proj,
                 VisADGeometryArray[] label_array, float[] f_array,
                 int flag) {
    this.trans = trans;
    this.proj = proj;
  ...
  public void controlChanged(double first_scale, double[] scale_a) {
    ...
    if (flag == 0) { //-- label
      ...
      double k = (f_array[0] / (f_array[1] - f_array[0])) *
                 (f_array[1] / f_array[0] - first_scale / scale_a[0]) *
                 scale_a[0];
      factor = k / scale_a[0];
      f_scale = (float)((scale_a[0] - k) / scale_a[0]);
    }

    Vector3f trans_vec = new Vector3f(f_scale * anchr_vertex[0],
