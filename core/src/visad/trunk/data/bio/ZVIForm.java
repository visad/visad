...
/**
 * ZVIForm is the new VisAD data adapter for Zeiss ZVI (.zvi) files.
 * @author Melissa Linkert linkert at cs.wisc.edu
 * @author Curtis Rueden ctrueden at wisc.edu
 */
public class ZVIForm extends Form implements FormBlockReader,
  FormFileInformer, FormProgressInformer, MetadataReader, OMEReader
{
  ...
  /** An instance of the old ZVI form, for use if this one fails. */
  private LegacyZVIForm legacy;

  /** Flag indicating the current file requires the legacy ZVI form. */
  private boolean needLegacy;

  private String currentId; // current filename
  private Hashtable metadata; // hashtable containing metadata
  private double percent; // percent complete
  private Object ome;
  ...
  /**
   * Opens an existing ZVI file from the given filename.
   *
   * @return VisAD Data object containing the ZVI data
   */
  public DataImpl open(String id)
    throws BadFormException, IOException, VisADException
  {
    if (noPOI) return legacy.open(id);

    if (!id.equals(currentId)) initFile(id);
    percent = 0;
    FieldImpl[] fields = new FieldImpl[getBlockCount(id)];
    ...
    if (fields.length == 1) data = fields[0];
    else {
      // combine data stack into time function
      RealType time = RealType.getRealType("time");
      FunctionType timeFunction =
        new FunctionType(time, fields[0].getType());
      Integer1DSet timeSet = new Integer1DSet(fields.length);
  ...
  /**
   * Saves a VisAD Data object to ZVI format at the given location.
   *
  ...
  /**
   * Adds data to an existing ZVI file
  ...
  /**
   * Opens an existing ZVI file from the given URL.
   *
   * @return VisAD Data object containing ZVI data
   * @exception UnimplementedException Always thrown (this method not
   * implemented).
   */
  public DataImpl open(URL url)
    throws BadFormException, IOException, VisADException
  {
    throw new UnimplementedException("ZVIForm.open(URL)");
  }
  ...
  public DataImpl open(String id, int block_number)
    throws BadFormException, IOException, VisADException
  {
    if (noPOI) return legacy.open(id, block_number);
    if (!id.equals(currentId)) initFile(id);
    if (needLegacy) return legacy.open(id, block_number);
    ...
    byte[] imageHead = (byte[]) headerData.get(new Integer(block_number));

    int pointer = 14;
    int numBytes = TiffTools.bytesToInt(imageHead, pointer, 2, true);
    pointer += 2 + numBytes;

    pointer += 2;
    int width = TiffTools.bytesToInt(imageHead, pointer, 4, true);
    pointer += 4 + 2;

    int height = TiffTools.bytesToInt(imageHead, pointer, 4, true);
    pointer += 4 + 2;

    int depth = TiffTools.bytesToInt(imageHead, pointer, 4, true);
    pointer += 4 + 2;

    int pixelFormat = TiffTools.bytesToInt(imageHead, pointer, 4, true);
    pointer += 4 + 2;

    pointer += 6; // count field is always 0

    int validBPP = TiffTools.bytesToInt(imageHead, pointer, 4, true);
    pointer += 4 + 2;
    ...
    byte[] px = (byte[]) pixelData.get(new Integer(block_number));
    ...
    if (bitsPerSample[0] == 8) {
      ...
      for (int i=0; i<4*numSamples; i++) {
        int q = (int) TiffTools.bytesToShort(px, i, 1, true);
        ...
        if (i < numSamples) samples[0][i] = (float) q;
          ...
          samples[3][i%numSamples] = (float) q;
        }
      }
    }
    ...
    try {
      ...
      Linear2DSet fieldSet = new Linear2DSet(domain, 0.0, width - 1.0,
          (int) width, height - 1.0, 0.0, (int) height);
  ...
  public int getBlockCount(String id)
    throws BadFormException, IOException, VisADException
  {
    if (noPOI) return legacy.getBlockCount(id);
    if (!id.equals(currentId)) initFile(id);
    return needLegacy ? legacy.getBlockCount(id) : nImages;
  }
  ...
  /** Checks if the given string is a valid filename for a ZVI file. */
  ...
  /** Returns the default file suffixes for ZVI file format. */
  ...
  /** Gets the percenteage complete of the form's current operation. */
  public double getPercentComplete() {
    if (noPOI) return legacy.getPercentComplete();
    return percent;
  }
  ...
  public Object getMetadataValue(String id, String field)
    throws BadFormException, IOException, VisADException
  {
    if (noPOI) return legacy.getMetadataValue(id, field);
    if (!id.equals(currentId)) initFile(id);
  ...
  public Hashtable getMetadata(String id)
    throws BadFormException, IOException, VisADException
  {
    if (noPOI) return legacy.getMetadata(id);
    if (!id.equals(currentId)) initFile(id);
    return metadata;
  }
  ...
  protected void initMetadata() {
    ...
    int pt = 14;
    int numBytes = TiffTools.bytesToInt(header, pt, 2, true);
    pt += 2 + numBytes;
    ...
    pt += 6;
    pt += 6;
    int pixel = TiffTools.bytesToInt(header, pt, 4, true);
    pt += 6;

    String format;
    switch (pixel) {
      case 1: format = "8-bit RGB Triple (B, G, R)"; break;
      case 2: format = "8-bit RGB Quad (B, G, R, A)"; break;
      case 3: format = "8-bit grayscale"; break;
      case 4: format = "16-bit signed integer"; break;
      case 5: format = "32-bit integer"; break;
      case 6: format = "32-bit IEEE float"; break;
      case 7: format = "64-bit IEEE float"; break;
      case 8: format = "16-bit unsigned RGB Triple (B, G, R)"; break;
      case 9: format = "32-bit RGB Triple (B, G, R)"; break;
    ...
    pt = 0;
    pt += 16;
    int w = TiffTools.bytesToInt(tags, pt, 2, true);
    pt += 2;

    int version = TiffTools.bytesToInt(tags, pt, 4, true);
    pt += 4;
    w = TiffTools.bytesToInt(tags, pt, 2, true);
    pt += 2;
    int numTags = TiffTools.bytesToInt(tags, pt, 4, true);
    pt += 4;
    ...
    for (int i=0; i<numTags; i++) {
      ...
      int type = TiffTools.bytesToInt(tags, pt, 2, true);
      pt += 2;
      ...
      switch (type) {
        case 2: // VT_I2: 16 bit integer
          data = new Integer(TiffTools.bytesToInt(tags, pt, 2, true));
          pt += 2;
          break;
        case 3: // VT_I4: 32 bit integer
          data = new Integer(TiffTools.bytesToInt(tags, pt, 4, true));
          pt += 4;
          break;
        case 4: // VT_R4: 32 bit float
          data = new Float(Float.intBitsToFloat(
            TiffTools.bytesToInt(tags, pt, 4, true)));
          pt += 4;
          break;
        case 5: // VT_R8: 64 bit float
          data = new Double(Double.longBitsToDouble(
            TiffTools.bytesToLong(tags, pt, 8, true)));
          pt += 8;
          break;
        ...
        case 8: // VT_BSTR: streamed storage object
          length = TiffTools.bytesToInt(tags, pt, 2, true);
          pt += 2;
          data = new String(tags, pt, length);
          pt += length;
          break;
        case 11: // VT_BOOL: 16 bit integer (true if !0)
          int temp = TiffTools.bytesToInt(tags, pt, 4, true);
          data = new Boolean(temp != 0);
          pt += 4;
          break;
        case 16: // VT_I1: 8 bit integer
          data = new Integer(TiffTools.bytesToInt(tags, pt, 1, true));
          pt += 1;
          break;
        ...
        case 20: // VT_I8: 64 bit integer
          data = new Integer(TiffTools.bytesToInt(tags, pt, 8, true));
          pt += 8;
          break;
        ...
        case 65: // VT_BLOB: binary data
          length = TiffTools.bytesToInt(tags, pt, 4, true);
          pt += 4;
          try {
            data = new String(tags, pt, length);
          }
      ...
      // read in tag ID
      int tagID = TiffTools.bytesToInt(tags, pt, 4, true);
      pt += 4;
    ...
    while (iter.hasNext()) {
      ...
      if (isInstance) parseDir(depth + 1, r.getVar("entry"));
        ...
        if (isContents && isImage) {
          ...
          length = TiffTools.bytesToInt((byte[])
            r.getVar("data"), pointer, 2, true);
          pointer += 4;
          pointer += length; // don't need these bytes

          int width = TiffTools.bytesToInt((byte[])
            r.getVar("data"), pointer, 4, true);
          imageWidth = width;
          pointer += 4 + 2;
          int height = TiffTools.bytesToInt((byte[])
            r.getVar("data"), pointer, 4, true);
          imageHeight = height;
          pointer += 4 + 2;
          pointer += 4 + 2;

          int format = TiffTools.bytesToInt((byte[])
            r.getVar("data"), pointer, 4, true);
          pointer += 4 + 2;
          ...
          dirName.substring(0, 4).equals("Item"))
        {
          ...
          byteCount += 2;
          int numBytes = TiffTools.bytesToInt((byte[])
          ...
          numBytes = TiffTools.bytesToInt((byte[])
            r.getVar("data"), byteCount, 2, true);
  ...
  {
    BaseTiffForm.testRead(new ZVIForm(), "ZVI", args);
  }
